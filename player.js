...
function setupPlayerControls(scene, player, camera) {
  ...
  canvas.addEventListener("click", () => {
    ...
    if (
      ...
      Date.now() - lastShootTime > shootCooldown
    ) {
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    const horizontalDirection = new BABYLON.Vector3(
      moveDirection.x,
      0,
      moveDirection.z,
    );
    ...
    camera.rotation.Quaternion = camera.rotationQuaternion =
...
function isPlayerOnGround(player) {
  ...
  const directions = [
    ...
    new BABYLON.Vector3(footRadius, 0, 0)
      .normalize()
      .add(BABYLON.Vector3.Down())
      .normalize(),
    new BABYLON.Vector3(-footRadius, 0, 0)
      .normalize()
      .add(BABYLON.Vector3.Down())
      .normalize(),
    new BABYLON.Vector3(0, 0, footRadius)
      .normalize()
      .add(BABYLON.Vector3.Down())
      .normalize(),
    new BABYLON.Vector3(0, 0, -footRadius)
  ...
  for (const dir of directions) {
    ...
    const hit = scene.pickWithRay(
      ray,
      (mesh) => mesh !== player && mesh.isPickable,
    );
    if (
      hit.hit &&
      hit.getNormal &&
...
function checkClip(player, camera, scene, baseDirection, offsets) {
  // deprecated function but could be used for hit detection on enemys if too close
  const rayLength = 1;
  ...
  for (const offset of offsets) {
    for (let i = 0; i < 20; i++) {
      ...
      const hit = scene.pickWithRay(
        ray,
        (mesh) => mesh !== player && mesh !== camera && !mesh.isRayLine,
      );
